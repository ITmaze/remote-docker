...
- ``docker.ssh.cmd``, the underlying ssh command that actually sends the docker commands to the *docker-machine*. You can edit the 'userName' and 'hostName' variables here. You cannot use a redirect to send commands across STDIN.
- ``docker.ssh.pipe.cmd``, use this command instead of ``docker.ssh.cmd`` to redirect commands across STDIN.
